...
use App\Mail\Reciveconfirm;

use App\Currency;
use App\CoinType;

class UserTestApiController extends Controller
{
    ...
    {  
        try{
            ...
            if($request->network=='BTC' || $request->network=='ETH'){ // -------- BTC and ETH coin     
            ...
            elseif($request->network=='LIO'){ // ------------------------- LIO coin
                ...
                /*$client = new Client;   
                $lio_usd_api = $client->get('https://btc-alpha.com/api/v1/ticker/?format=json&pair=LIO_USD');
                $lio_usd_details = json_decode($lio_usd_api->getBody(),true);
                $currency_value = $lio_usd_details[0]['last'];*/
                $currency_value =Setting::get('coin_value');

                if($currency=='EUR'){

                    /*$client = new Client;   
                    $usd_eur = $client->get('https://api.exchangeratesapi.io/latest?base=USD');
                    $usd_eur_details = json_decode($usd_eur->getBody(),true);
                    $usd_eur_value = $usd_eur_details['rates']['EUR'];

                    $lio_usd = $currency_value;
                    $lio_eur = ($lio_usd/1)*$usd_eur_value;
                    $currency_value = $lio_eur;*/
                    
                    $currency_value = Setting::get('coin_eur_value');
                }
    ...
    public function ethgas(){
        
        try{

            $gas=[];
            $client_gas = new Client(); 
            $res_gas = $client_gas->get('https://ethgasstation.info/json/ethgasAPI.json');
            $gas_price_temp = json_decode($res_gas->getBody(),true);

            if(isset($gas_price_temp['average'])){
                
                /*return $price_num1=\DB::select('select ('.$gas_price_temp['fastest'].' / 10000000000)');
                $arr1 = json_decode(json_encode($price_num1[0]), true);
                $final_num1=current($arr1);
                $gas['fastest']=$final_num1;*/
                $gas['fastest']=[
                    'eth'=> $gas_price_temp['fastest'] / 10000000000,
                    'value'=>$gas_price_temp['fastest'],
                ];    
               
                /*$price_num2=\DB::select('select ('.$gas_price_temp['fast'].')/10000000000');
                $arr2 = json_decode(json_encode($price_num2[0]), true);
                $final_num2=current($arr2);
                $gas['fast']=$final_num2;*/
                $gas['fast']=[
                    'eth'=>$gas_price_temp['fast']/10000000000,
                    'value'=>$gas_price_temp['fast'],
                ];    

                /*$price_num3=\DB::select('select '.$gas_price_temp['average'].'/10000000000');
                $arr3 = json_decode(json_encode($price_num3[0]), true);
                $final_num3=current($arr3);
                $gas['average']=$final_num3;*/
                $gas['average']=[
                    'eth'=>$gas_price_temp['average']/10000000000,
                    'value'=>$gas_price_temp['average'],
                ];    

                /*$price_num4=\DB::select('select '.$gas_price_temp['safeLow'].'/10000000000');
                $arr4 = json_decode(json_encode($price_num4[0]), true);
                $final_num4=current($arr4);
                $gas['safelow']=$final_num4;*/
                $gas['safelow']=[
                    'eth'=> $gas_price_temp['safeLow']/10000000000,
                    'value'=> $gas_price_temp['safeLow'],
                ];
            }

            return response()->json(['gas' => $gas], 200);
        }catch(Exception $e){
            return response()->json(['error' => trans('api.something_went_wrong')], 500);
        }
    ...
    public function sendcoin(Request $request){
        ...
        try{
            ...
            $current_gas_limit = $current_gas_price ="";
           ...
           // return $request->from_address;
            ...
            if($request->has('from_address'))
            {
              ...
              $param=[$name];
                ...
                $curldata=$this->npmcurl($body);
                $name = strtolower($curldata['result']);
            }
            else
            {
              $name = strtolower($user['email']);  
            ...
            if($user->network == 'ETH'){
                ...
                $body = ["senderAddress" => $user->eth_address,"senderPassword" => $name,"amount" =>$request->amount,"receiverAddress" => $request->to_address,"gasPrice"=>$request->eth_gas_value];

                $url ="http://localhost:3005/sendEther";
                $res = $client->post($url, [
                    'headers' => $headers,
                    'body' => json_encode($body),
                ]);
                ...
                if(isset($res['transactionHash'])) {
                    $curldata['result']=$res['transactionHash'];
                }else{
            ...
            if($user->network == 'ETHtest'){
                ...
                if(isset($unlock['result'])) {
                    ...
                    //$price_num = \DB::select('select ('.$request->amount.') * 1000000000000000000');
                    $arr = json_decode(json_encode($price_num[0]), true);
                    $final_num=current($arr);
                    $price='0x'.dechex($final_num);
                    ...
                    if(isset($gas_price_temp['average'])){ 
                        ...
                        if($request->eth_gas_index=='fastest'){
                            $current_gas_price='0x'.dechex(($gas_price_temp['fastest']/10) * 1000000000);
                        }
                        if($request->eth_gas_index=='fast'){
                            $current_gas_price='0x'.dechex(($gas_price_temp['fast']/10) *1000000000);
                        }    
                        if($request->eth_gas_index=='average'){
                            $current_gas_price='0x'.dechex(($gas_price_temp['average']/10) *1000000000);
                        }
                        if($request->eth_gas_index=='safelow'){
                            $current_gas_price='0x'.dechex(($gas_price_temp['safeLow']/10) *1000000000);
                        } 

                    }*/
                    
                    //$current_gas_price='0x'.dechex(($request->eth_gas_value/10)*1000000000);
                    $current_gas_price='0x'.dechex(($request->eth_gas_value)* 1000000000000000000);
                    ...
                    //$body = ["jsonrpc" => "2.0","method" => "eth_sendTransaction", "params" => array(["from" => $user->eth_address, "to" => $request->to_address, "gas" => "0x5208", "gasPrice" => $current_gas_price, "value" => $price]), "id" => 1];
                    ...
                    $curldata['result']=$details['result'];
    ...
    public function history(Request $request)
    {
        try{
            ...
            if($user->network == 'LIO'){
                ...
                $curldata['result'] = array_reverse($history);
    ...
    public function tagwrite(Request $request)
    {

        //$address = "AaSZDP6KXfZUqTU4e94AdzZ5sz35qHyRpm";//getaccount
        //AP3cKhHPnhpCrdqe91wAhEioGFWkE2opkd
        //AaSZDP6KXfZUqTU4e94AdzZ5sz35qHyRpm
    ...
    public function testGetbalance(Request $request)
    {
        ...
        $name = strtolower($curldata1['result']);

        $param=[$name,1];
            $body = [                  
                    'params' => $param,
                    'method' => 'getbalance'/*'getreceivedbyaddress'*/,        
                ];

        $curldata=$this->npmcurl($body);

        return response()->json(['balance' =>$curldata['result']], 200);
    }


    public function recive_mail(Request $request)
    {
        $otp = rand(9999,99999);
        $name = $request->address;
        $param1=[$name];
        ...
        $name = strtolower($curldata1['result']);
        Mail::to($name)->send(new Reciveconfirm($otp));
        return response()->json(['Otp' =>$otp,'address'=>$name], 200);
    }
